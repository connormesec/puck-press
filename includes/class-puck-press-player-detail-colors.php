...
class Puck_Press_Player_Detail_Colors
{
    ...
    public static function get_default_fonts(): array { return [ 'player-font' => '' ]; }
    public static function get_font_labels(): array   { return [ 'player-font' => 'Player Page Font' ]; }

    public static function get_colors(): array
    {
        $saved = get_option( self::COLORS_OPTION, [] );
        return array_merge( self::get_default_colors(), is_array( $saved ) ? $saved : [] );
    }

    public static function get_fonts(): array
    {
        $saved = get_option( self::FONTS_OPTION, [] );
        return array_merge( self::get_default_fonts(), is_array( $saved ) ? $saved : [] );
    }
