...
class Puck_Press_Yoast_Sitemap
{
    ...
    {
        add_action( 'admin_notices',              [ self::class, 'maybe_show_yoast_notice' ] );
        add_action( 'admin_notices',              [ self::class, 'maybe_show_permalink_notice' ] );
        add_action( 'admin_post_pp_flush_permalinks', [ self::class, 'handle_flush_permalinks' ] );

        if ( ! self::is_yoast_active() ) {
            return;
        }

        add_filter( 'wpseo_sitemap_index',  [ self::class, 'add_to_sitemap_index' ] );
        ...
        add_action( 'init', [ self::class, 'register_sitemap_type' ], 20 );
    }
    ...
    public static function register_sitemap_type(): void
    {
        ...
        $wpseo_sitemaps->register_sitemap( 'players', [ self::class, 'serve_players_sitemap' ] );
    }
