...
class Puck_Press_Schedule_Source_Importer {
	...
	private function load_dependencies() {
		require_once plugin_dir_path( __DIR__ ) . 'schedule/class-puck-press-schedule-wpdb-utils.php';
		require_once plugin_dir_path( __DIR__ ) . 'class-puck-press-tts-api.php';
		require_once plugin_dir_path( __DIR__ ) . 'schedule/class-puck-press-schedule-process-acha-url.php';
		require_once plugin_dir_path( __DIR__ ) . 'schedule/class-puck-press-schedule-process-usphl-url.php';
		require_once plugin_dir_path( __DIR__ ) . 'schedule/class-puck-press-schedule-process-csv-data.php';
	...
	public function populate_raw_schedule_table_from_sources() {
		...
		$active_sources = $this->schedule_db_utils->get_active_schedule_sources( $this->schedule_id ) ?? array();
	...
	function apply_edits_and_save_to_display_table() {
		...
		$edit_map    = array(); // game_id => merged field overrides
		$delete_ids  = array(); // game_ids to exclude
		$insert_mods = array(); // full game arrays to inject (no raw record)
		$results     = array();
