...
Replace the bracketed title with your actual title. The very first character of your response must be [. Do not include any text before the opening bracket.',
	...
	private function assignTargetTeam( $gameData ) {
		...
		} elseif ( $visitingTeam['info']['id'] == $targetTeamId ) {
			return array(
	...
	private function determineHighlightedPlayer( $skaters ) {
		...
		$mostPtsPlayers = array_filter(
			$skaters,
			function ( $player ) use ( $maxPoints ) {
		...
		if ( count( $mostPtsPlayers ) > 1 ) {
			...
			$mostGoalsPlayers = array_filter(
				$skaters,
				function ( $player ) use ( $maxGoals ) {
					return $player['stats']['goals'] == $maxGoals;
				}
			);

			$maxAssists         = max( array_column( array_column( $skaters, 'stats' ), 'assists' ) );
			$mostAssistsPlayers = array_filter(
				$skaters,
				function ( $player ) use ( $maxAssists ) {
			...
			if ( $mostAssistsPlayer['stats']['assists'] > $mostGoalsPlayer['stats']['goals'] ) {
			...
			} elseif ( $mostGoalsPlayer['stats']['goals'] == 1 ) {
				return array(
				...
				$mostPenaltiesPlayers = array_filter(
					$skaters,
					function ( $player ) use ( $maxPenalties ) {
