...
class Parse_Usphl_Score_Sheet {
	...
	public function parseScoreSheet( $gameId ) {
		...
		if ( is_wp_error( $response ) ) {
			wp_send_json_error( array( 'message' => 'Failed to fetch game page: ' . $response->get_error_message() ) );
		}

		$html = wp_remote_retrieve_body( $response );
		if ( ! $html ) {
			wp_send_json_error( array( 'message' => 'Failed to retrieve game page content' ) );
		}
		...
		if ( $bodyContent->length === 0 ) {
			wp_send_json_error( array( 'message' => 'Invalid game ID - no game data found' ) );
		}
		...
		if ( $gameInfoCells->length ) {
			...
			wp_send_json_error( array( 'message' => 'Invalid game - no date information found' ) );
		}
		...
		if ( $data['game_status'] === 'completed' ) {
			...
			$data['visitor']['players']       = array();
			$data['visitor']['goalies']       = array();
			$data['visitor']['special_teams'] = array();
			$data['visitor']['total_shots']   = 0;

			$data['home']['players']       = array();
			$data['home']['goalies']       = array();
			$data['home']['special_teams'] = array();
