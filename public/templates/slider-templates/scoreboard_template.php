...
class ScoreboardTemplate extends PuckPressTemplate
{
    ...
    public function render_with_options(array $games, array $options): string
    {
        ...
        ob_start();
        ?>
        <div class="scoreboard_slider_container pp-sb-container">
            <div class="pp-sb-outer">
                ...
                <a class="pp-sb-cal-btn" href="<?= esc_url( $cal_url ) ?>" aria-label="View full schedule">
        ...
        <script>window.ppScoreboardIndex = <?= (int) $scroll_to ?>;</script>
        <?php
        return ob_get_clean();
    }
    ...
    private function render_card(array $game): string
    {
        ...
        ob_start();
        ?>
        <div class="pp-sb-card<?= !empty($ticket_url) ? ' pp-sb-card--has-ticket' : '' ?>">

            <div class="pp-sb-card-hdr">
                <span class="pp-sb-date-label"><?= esc_html($date_label) ?></span>
                <span class="pp-sb-sep" aria-hidden="true">•</span>
                <span class="pp-sb-status"><?= esc_html($status_label) ?></span>
            </div>

            <div class="pp-sb-teams">
                <div class="pp-sb-team-row">
                    <img class="pp-sb-logo" src="<?= esc_url($target_logo) ?>" alt="<?= esc_attr($target_name) ?>" loading="lazy">
                    <span class="pp-sb-name"><?= esc_html($target_name) ?></span>
                    <?php if ($has_score): ?>
                        <span class="pp-sb-score<?= $target_wins ? ' pp-sb-score--w' : '' ?>"><?= esc_html($game['target_score']) ?></span>
                    <?php endif; ?>
                </div>
                <div class="pp-sb-team-row">
                    <img class="pp-sb-logo" src="<?= esc_url($opponent_logo) ?>" alt="<?= esc_attr($opponent_name) ?>" loading="lazy">
                    <span class="pp-sb-name"><?= esc_html($opponent_name) ?></span>
                    <?php if ($has_score): ?>
                        <span class="pp-sb-score<?= $opponent_wins ? ' pp-sb-score--w' : '' ?>"><?= esc_html($game['opponent_score']) ?></span>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="pp-sb-overlay">
                <div class="pp-sb-ovl-hdr">
                    <span class="pp-sb-ovl-date"><?= esc_html($date_label) ?></span>
                    <span class="pp-sb-ovl-sep" aria-hidden="true">•</span>
                    <span class="pp-sb-ovl-time"><?= esc_html($status_label) ?></span>
                </div>
                <?php if (!empty($venue)): ?>
                    <div class="pp-sb-ovl-venue"><?= esc_html($venue) ?></div>
                <?php endif; ?>
                <div class="pp-sb-ovl-links">
                    <a class="pp-sb-ovl-link" href="<?= esc_url($ticket_url) ?>" target="_blank" rel="noopener noreferrer" aria-label="Buy tickets">
