...
class ArenaTemplate extends PuckPressTemplate {
	...
	private function renderBanner( ?array $game ): string {
		...
		foreach ( array( array( 'days', 'Days' ), array( 'hours', 'Hrs' ), array( 'mins', 'Mins' ), array( 'secs', 'Secs' ) ) as [$key, $lbl] ) {
	...
	private function renderRecordBar( array $past_games ): string {
		...
		$stats = array(
			array( 'Record', "{$wins}-{$losses}-{$ties}" ),
			array( 'Home', "{$home_w}-{$home_l}-{$home_t}" ),
			array( 'Away', "{$away_w}-{$away_l}-{$away_t}" ),
			array( 'GF', (string) $gf ),
			array( 'GA', (string) $ga ),
		);
