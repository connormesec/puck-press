...
class ConferenceTemplate extends PuckPressTemplate
{
    ...
    {
        return in_array($status, ['Final', 'Final OT', 'Final/OT', 'Final SO', 'Final/SO'], true);
    }
    ...
    {
        $teams = [$normalized['home_team_name'], $normalized['away_team_name']];
        sort($teams);
    ...
    {
        $records        = [];
        $final_statuses = ['Final', 'Final OT', 'Final/OT', 'Final SO', 'Final/SO'];
        ...
        foreach ($games as $game) {
            ...
            foreach ([$game['home_team_name'], $game['away_team_name']] as $team) {
                if (!isset($records[$team])) {
                    $records[$team] = ['w' => 0, 'l' => 0, 't' => 0];
                }
            }
    ...
    private function group_by_date(array $games): array
    {
        ...
        foreach ($grouped as &$day_games) {
            usort($day_games, function (array $a, array $b): int {
