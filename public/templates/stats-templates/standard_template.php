...
class StandardTemplate extends PuckPressTemplate
{
    ...
    private function buildSkatersSection(array $skaters, array $col): string
    {
        ...
        if (!empty($col['show_pts_per_game'])) $html .= '<th class="pp-stats-col-opt" data-tip="Points Per Game">Pts/GP</th>';
        if (!empty($col['show_pim']))          $html .= '<th class="pp-stats-col-opt" data-tip="Penalty Minutes">PIM</th>';
        if (!empty($col['show_ppg']))          $html .= '<th class="pp-stats-col-opt" data-tip="Power Play Goals">PPG</th>';
        if (!empty($col['show_shg']))          $html .= '<th class="pp-stats-col-opt" data-tip="Short-Handed Goals">SHG</th>';
        if (!empty($col['show_gwg']))          $html .= '<th class="pp-stats-col-opt" data-tip="Game-Winning Goals">GWG</th>';
        if (!empty($col['show_sh_pct']))       $html .= '<th class="pp-stats-col-opt" data-tip="Shooting Percentage">SH%</th>';
    ...
    private function buildGoaliesSection(array $goalies, array $col): string
    {
        ...
        if (!empty($col['show_goalie_otl']))   $html .= '<th class="pp-stats-col-opt" data-tip="Overtime Losses">OTL</th>';
        if (!empty($col['show_goalie_gaa']))   $html .= '<th class="pp-stats-col-opt" data-tip="Goals Against Average">GAA</th>';
        if (!empty($col['show_goalie_svpct'])) $html .= '<th class="pp-stats-col-opt" data-tip="Save Percentage">SV%</th>';
        if (!empty($col['show_goalie_sa']))    $html .= '<th class="pp-stats-col-opt" data-tip="Shots Against">SA</th>';
        if (!empty($col['show_goalie_saves'])) $html .= '<th class="pp-stats-col-opt" data-tip="Total Saves">Saves</th>';
