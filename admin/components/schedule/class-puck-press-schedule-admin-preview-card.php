...
class Puck_Press_Schedule_Admin_Preview_Card extends Puck_Press_Admin_Preview_Card_Abstract
{
    ...
    protected function make_template_manager() { return new Puck_Press_Schedule_Template_Manager($this->schedule_id); }
    ...
    public function get_all_templates_html()
    {
        ...
        foreach ($this->templates as $template) {
            $output .= $template->render_with_options($this->data, ['schedule_id' => $this->schedule_id]);
        }
        return $output;
    }
    ...
    public function get_template_html($template_name)
    {
        foreach ($this->templates as $template) {
            if ($template->get_key() === $template_name) {
                return $template->render_with_options($this->data, ['schedule_id' => $this->schedule_id]);
            }
        }
