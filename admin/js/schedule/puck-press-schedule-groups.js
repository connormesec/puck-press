(function ($) {
    jQuery(document).ready(function ($) {
        const admin     = window.ppScheduleAdmin || {};
        const nonce     = admin.nonce || '';
        const domainLc  = 'schedule';
        ...
        $('#pp-schedule-group-selector').on('change', function () {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'pp_set_active_schedule_id',
                ...
                error: () => {
                    alert('Failed to switch schedule group.');
                }
            });
        });
        ...
        $(document).on('click', '.pp-delete-group-btn', function () {
            ...
            if (!confirm('Delete this schedule group and all its data? This cannot be undone.')) {
                return;
            }
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response.success) {
                        ...
                        // If active schedule was deleted, reload to reset
                        if (parseInt(groupId, 10) === parseInt(admin.activeScheduleId, 10)) {
                            window.location.reload();
                        }
                    } else {
